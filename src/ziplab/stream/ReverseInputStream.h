#ifndef ZIPLAB_STREAM_REVERSE_INPUTSTREAM_HPP
#define ZIPLAB_STREAM_REVERSE_INPUTSTREAM_HPP
...
class BasicReverseInputStream : public BasicIOStreamRoot<MemoryBufferT, CharT, Traits>
{
public:
    ...
    using this_type     = BasicReverseInputStream<buffer_type, char_type, traits_type>;
...
public:
    BasicReverseInputStream(buffer_type & buffer) : super_type(buffer), pos_(0) {
    }

    BasicReverseInputStream(const BasicReverseInputStream & src)
        : super_type(src.buffer()), pos_(src.pos()) {
    }
    BasicReverseInputStream(BasicReverseInputStream && src)
        : super_type(std::forward<memory_buffer_t>(src.buffer())),
          pos_(src.pos()) {
    }

    ~BasicReverseInputStream() {
        //destroy();
    }
    ...
    void copy(const BasicReverseInputStream & src) {
        if (std::addressof(src) != this) {
            copy_data(src);
        }
    }

    void swap(BasicReverseInputStream & other) {
        if (std::addressof(other) != this) {
            swap_data(other);
        }
    }

    friend inline void swap(BasicReverseInputStream & lhs, BasicReverseInputStream & rhs) {
        lhs.swap(rhs);
    }
    ...
    // Unsafe write
    void unsafeWrite(const BasicReverseInputStream & out) {
        this->unsafeWrite(out.data(), out.size());
    }

    // Safety write
    bool write(const BasicReverseInputStream & out) {
...
protected:
    inline void copy_data(const BasicReverseInputStream & src) {
        super_ref(this).copy_data(super_ref(&src));
        this->pos_ = src.pos_;
    }

    inline void swap_data(BasicReverseInputStream & other) {
...
using ReverseInputStream  = BasicReverseInputStream< BasicMemoryBuffer<char, true, std::char_traits<char> >, char, std::char_traits<char>>;
using WReverseInputStream = BasicReverseInputStream< BasicMemoryBuffer<wchar_t, true, std::char_traits<wchar_t> >, wchar_t, std::char_traits<wchar_t>>;

using ReverseInputStreamView  = BasicReverseInputStream< BasicMemoryView<char, std::char_traits<char> >, char, std::char_traits<char>>;
using WReverseInputStreamView = BasicReverseInputStream< BasicMemoryView<wchar_t, std::char_traits<wchar_t> >, wchar_t, std::char_traits<wchar_t>>;
...
template <typename MemoryBufferT, typename CharT, typename Traits = std::char_traits<CharT>>
inline void swap(ziplab::BasicReverseInputStream<MemoryBufferT, CharT, Traits> & lhs,
                 ziplab::BasicReverseInputStream<MemoryBufferT, CharT, Traits> & rhs) {
    lhs.swap(rhs);
}

} // namespace std

#endif // ZIPLAB_STREAM_REVERSE_INPUTSTREAM_HPP
