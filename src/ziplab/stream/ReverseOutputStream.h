#ifndef ZIPLAB_STREAM_REVERSE_OUTPUTSTREAM_HPP
#define ZIPLAB_STREAM_REVERSE_OUTPUTSTREAM_HPP
...
class BasicReverseOutputStream : public BasicIOStreamRoot<MemoryBufferT, CharT, Traits>
{
public:
    ...
    using this_type     = BasicReverseOutputStream<buffer_type, char_type, traits_type>;
...
public:
    BasicReverseOutputStream(buffer_type & buffer) : super_type(buffer) {
    }

    BasicReverseOutputStream(const BasicReverseOutputStream & src)
        : super_type(src.super()) {
    }
    BasicReverseOutputStream(BasicReverseOutputStream && src)
        : super_type(std::forward<super_type>(src.super())) {
    }

    ~BasicReverseOutputStream() {
        //destroy();
    }
    ...
    void copy(const BasicReverseOutputStream & src) {
        if (std::addressof(src) != this) {
            copy_data(src);
        }
    }

    void swap(BasicReverseOutputStream & other) {
        if (std::addressof(other) != this) {
            swap_data(other);
        }
    }

    friend inline void swap(BasicReverseOutputStream & lhs, BasicReverseOutputStream & rhs) {
        lhs.swap(rhs);
    }
    ...
    // Unsafe write
    void unsafeWrite(const BasicReverseOutputStream & out) {
        this->unsafeWrite(out.data(), out.size());
    }

    // Safety write
    bool write(const BasicReverseOutputStream & out) {
...
protected:
    ...
    inline void copy_data(const BasicReverseOutputStream & src) {
        super_ref(this).copy_data(super_ref(&src));
    }

    inline void swap_data(BasicReverseOutputStream & other) {
...
using ReverseOutputStream  = BasicReverseOutputStream< BasicMemoryBuffer<char, true, std::char_traits<char> >, char, std::char_traits<char>>;
using WReverseOutputStream = BasicReverseOutputStream< BasicMemoryBuffer<wchar_t, true, std::char_traits<wchar_t> >, wchar_t, std::char_traits<wchar_t>>;

using ReverseOutputStreamView  = BasicReverseOutputStream< BasicMemoryView<char, std::char_traits<char> >, char, std::char_traits<char>>;
using WReverseOutputStreamView = BasicReverseOutputStream< BasicMemoryView<wchar_t, std::char_traits<wchar_t> >, wchar_t, std::char_traits<wchar_t>>;
...
template <typename MemoryBufferT, typename CharT, typename Traits = std::char_traits<CharT>>
inline void swap(ziplab::BasicReverseOutputStream<MemoryBufferT, CharT, Traits> & lhs,
                 ziplab::BasicReverseOutputStream<MemoryBufferT, CharT, Traits> & rhs) {
    lhs.swap(rhs);
}

} // namespace std

#endif // ZIPLAB_STREAM_REVERSE_OUTPUTSTREAM_HPP
