#ifndef ZIPLAB_STREAM_SEQUENTIAL_INPUTSTREAM_HPP
#define ZIPLAB_STREAM_SEQUENTIALL_INPUTSTREAM_HPP
...
class BasicSequentialInputStream : public BasicIOStreamRoot<MemoryBufferT, CharT, Traits>
{
public:
    ...
    using this_type     = BasicSequentialInputStream<buffer_type, char_type, traits_type>;
...
public:
    BasicSequentialInputStream(buffer_type & buffer) : super_type(buffer), pos_(0) {
    }

    BasicSequentialInputStream(const BasicSequentialInputStream & src)
        : super_type(src.buffer()), pos_(src.pos()) {
    }
    BasicSequentialInputStream(BasicSequentialInputStream && src)
        : super_type(std::forward<memory_buffer_t>(src.buffer())),
          pos_(src.pos()) {
    }

    ~BasicSequentialInputStream() {
        //destroy();
    }
    ...
    void copy(const BasicSequentialInputStream & src) {
        if (std::addressof(src) != this) {
            copy_data(src);
        }
    }

    void swap(BasicSequentialInputStream & other) {
        if (std::addressof(other) != this) {
            swap_data(other);
        }
    }

    friend inline void swap(BasicSequentialInputStream & lhs, BasicSequentialInputStream & rhs) {
        lhs.swap(rhs);
    }
    ...
    // Unsafe write
    void unsafeWrite(const BasicSequentialInputStream & out) {
        this->unsafeWrite(out.data(), out.size());
    }

    // Safety write
    bool write(const BasicSequentialInputStream & out) {
...
protected:
    inline void copy_data(const BasicSequentialInputStream & src) {
        super_ref(this).copy_data(super_ref(&src));
        this->pos_ = src.pos_;
    }

    inline void swap_data(BasicSequentialInputStream & other) {
...
using SequentialInputStream  = BasicSequentialInputStream< BasicMemoryBuffer<char, true, std::char_traits<char> >, char, std::char_traits<char>>;
using WSequentialInputStream = BasicSequentialInputStream< BasicMemoryBuffer<wchar_t, true, std::char_traits<wchar_t> >, wchar_t, std::char_traits<wchar_t>>;

using SequentialInputStreamView  = BasicSequentialInputStream< BasicMemoryView<char, std::char_traits<char> >, char, std::char_traits<char>>;
using WSequentialInputStreamView = BasicSequentialInputStream< BasicMemoryView<wchar_t, std::char_traits<wchar_t> >, wchar_t, std::char_traits<wchar_t>>;
...
template <typename MemoryBufferT, typename CharT, typename Traits = std::char_traits<CharT>>
inline void swap(ziplab::BasicSequentialInputStream<MemoryBufferT, CharT, Traits> & lhs,
                 ziplab::BasicSequentialInputStream<MemoryBufferT, CharT, Traits> & rhs) {
    lhs.swap(rhs);
}

} // namespace std

#endif // ZIPLAB_STREAM_SEQUENTIAL_INPUTSTREAM_HPP
